...
def test_batch_withdraw(chain, payer, owner, tokens, stream_manager):
    ...
    for i in range(stream_to_create):
    ...
    withdraw = stream_manager.batch_withdraw(payers, streams, sender=owner)
    assert len(withdraw.events["Withdrawn"]) == stream_to_create
